...
const NewPlayer = ({
  ...
  const [player, setPlayer] = useState<Player>({name: '', number: '', position: []})
  const [errors, setErrors] = useState<{[s: string]: string}>({})

  useEffect(() => {
    if (players.map(player => player.number).filter(num => typeof num !== 'undefined' && num !== '').includes(player.number)) {
        ...
        <div className="card">
          ...
          <div className="card-content">
            <form onSubmit={(e) => {
              ...
              <PlayerField index={0} errors={errors} player={player} updatePlayer={(player) => setPlayer(player)}/>
...
export default () => {
  ...
  return (
    <Layout>
      ...
      <NewPlayer 
        players={players}
        open={modalOpen} close={() => setModalOpen(!modalOpen)} save={(player) => {
        setPlayers([
        ...players,
        player,
      ])
